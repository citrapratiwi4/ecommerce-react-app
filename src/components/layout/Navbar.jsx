...
function Navbar({ wishlist = [], cart = [], setIsCartOpen = () => {} }) {
  ...
  return (
    <>
      <nav className="mb-6 sticky top-0 bg-white/80 backdrop-blur-md border-b border-gray-100 z-40 relative">
        ...
        <div className="max-w-[1400px] mx-auto px-5 py-5 md:py-6 flex items-center justify-between">
          ...
          <div className="flex-1 md:flex-none flex justify-center md:justify-start order-2 md:order-1">
            <Link
              to="/"
              className="text-xl md:text-2xl tracking-[0.15em] uppercase text-black font-normal"
            >
              ÉLANORA
            </Link>
          </div>

          {/* Navigation Links (Desktop) & Burger Toggle (Mobile) */}
          ...
          <div className="flex-1 md:flex-none flex justify-end gap-4 md:gap-5 items-center text-gray-800 order-3">
            ...
            <Link
              to="/account"
              className="hidden md:block hover:text-gray-500 transition"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
            ...
            <Link
              to="/wishlist"
              className="hidden md:block hover:text-gray-500 transition relative"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                      {searchResults.slice(0, 5).map((item) => (
                        ...
                        >
                          ...
                          <h4 className="text-[10px] font-medium tracking-[0.15em] uppercase text-gray-800 mb-1">
                            {item.name}
                          </h4>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-[85%] max-w-sm bg-white z-[70] transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <Link
            to="/"
            onClick={closeMobileMenu}
            className="text-xl tracking-[0.15em] uppercase text-black font-normal"
          >
            ÉLANORA
          </Link>
          <button
          ...
          <ul className="space-y-5 text-sm uppercase tracking-widest text-gray-600">
            ...
            <li>
              ...
              >
                ...
                Wishlist
                {wishlist.length > 0 && (
                  <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded-full ml-1">
                    {wishlist.length}
                  </span>
                )}
              </Link>
