...
// . Tambahkan isLoggedIn di sini
function SuccessPage({ wishlist, cart, setIsCartOpen, isLoggedIn }) {
  const navigate = useNavigate();
  
  // Membuat nomor pesanan acak untuk simulasi (misal: #5892)
    ...
    <div className="bg-white min-h-screen text-gray-800 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16 md:py-20 animate-fade-in-up">
        <div className="text-center max-w-md w-full">
          ...
          {/* . PERBAIKAN: Ikon Sukses dikecilkan proporsinya untuk layar HP */}
          ...
          {/* . PERBAIKAN: Pesan Sukses Utama diturunkan ukurannya di HP (text-2xl) */}
          <h1 className="text-2xl md:text-3xl font-light tracking-wide mb-3 md:mb-4 text-gray-900">
            Thank You for Your Order!
          </h1>
          
          {/* . PERBAIKAN: Teks paragraf dikecilkan sedikit (text-xs) dan diberi padding agar tidak mentok ke tepi layar HP */}
          <p className="text-xs md:text-sm text-gray-500 mb-8 md:mb-10 leading-relaxed px-2 md:px-0">
            Pesanan Anda dengan nomor <span className="font-semibold text-black">#{orderNumber}</span> telah berhasil dibuat. Kami sedang menyiapkan pesanan Anda dan akan mengirimkan email konfirmasi beserta rincian pengiriman.
          </p>

          {/* LOGIKA PINTAR: Cek apakah user sudah login atau belum */}
          {!isLoggedIn ? (
            /* JIKA GUEST (BELUM LOGIN): Tampilkan kotak promosi bikin akun */
            <div className="bg-[#fafafa] p-6 md:p-8 rounded-lg mb-8 md:mb-10 border border-gray-200 shadow-sm">
              ...
              <p className="text-[11px] md:text-xs text-gray-500 mb-5 md:mb-6 font-light leading-relaxed">
                Buat akun sekarang untuk memantau status pengiriman secara real-time dan menyimpan riwayat belanja Anda.
              </p>
              <button
            ...
            /* JIKA SUDAH LOGIN: Tampilkan tombol langsung ke Order History */
          ...
          {/* Tombol Kembali ke Home (Selalu ada) */}
          <Link
            to="/"
