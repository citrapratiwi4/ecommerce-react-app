  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 flex flex-col">
      ...
      <header className="w-full bg-white border-b border-gray-200 px-6 py-6 lg:px-16 xl:px-24 flex justify-between items-center z-10 sticky top-0">
        ...
        <Link to="/" className="text-[11px] text-gray-500 hover:text-black transition tracking-widest uppercase font-medium">
          Continue Shopping
        </Link>
      </header>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-6 py-10 md:py-12">
        ...
        {cart.length === 0 ? (
          <div className="text-center space-y-6 py-10">
            <p className="text-gray-500 font-light">Your shopping cart is currently empty.</p>
            <button 
              onClick={() => navigate("/")}
          ...
          <div className="w-full">
              ...
              {cart.map((item, index) => (
                ...
                >
                  ...
                  <div className="md:col-span-6 flex gap-4 md:gap-6 items-start">
                    ...
                    {/* Foto Produk */}
                    <Link to={`/product/${item.id}`} className="w-24 h-32 md:w-[100px] md:h-[135px] shrink-0 bg-gray-50 rounded overflow-hidden">
                      <img src={item.image} alt={item.name} className="w-full h-full object-cover mix-blend-multiply" />
                    </Link>
                    
                    {/*  Menghapus padding-top agar sejajar persis dengan tepi atas gambar */}
                    <div className="flex flex-col">
                      
                      {/* Nama Produk */}
                      <Link to={`/product/${item.id}`} className="text-xl md:text-2xl font-normal text-gray-900 mb-2 hover:underline leading-none -mt-1">
                        {item.name}
                      </Link>
                      
                      <p className="text-base text-gray-500 mb-2">Rp {item.price.toLocaleString("id-ID")}</p>
                  ...
                  <div className="md:col-span-4 flex justify-start md:justify-center mt-4 md:mt-0">
                    ...
                    {/* Wrapper ini menyatukan kotak Quantity & Ikon Sampah */}
                    {/* Ditambah -mt-1 agar sejajar atas dengan nama produk */}
                    <div className="flex items-center gap-4 md:-mt-1">
                      ...
                      <div className="flex items-center border border-gray-400 rounded overflow-hidden h-[38px] w-[100px] bg-white">
                        ...
                        >−</button>
                        <span className="w-1/3 text-center text-sm font-medium text-gray-800">{item.quantity}</span>
                        <button 
                      ...
                      {/* Ikon Sampah di sebelah kanan kotak Quantity */}
                      <button 
            ...
            {/* . PERBAIKAN: Baris Subtotal sekarang dimasukkan ke dalam Grid 12 kolom agar sejajar lurus ke bawah! */}
            <div className="hidden md:grid grid-cols-12 gap-6 mt-8 border-b border-gray-100 pb-6 items-end">
              <div className="col-span-6"></div> {/* Bagian kiri kosong (Di bawah produk) */}
              
              {/* Bacaan "Subtotal" sejajar lurus di bawah "Quantity" dan diperbesar */}
              <div className="col-span-4 flex justify-center">
                <span className="text-lg md:text-xl font-normal text-gray-900 tracking-wide">Subtotal</span>
              </div>
              
              {/* Harga Total sejajar lurus di ujung kanan */}
              <div className="col-span-2 flex justify-end">
                <span className="text-xl md:text-2xl font-normal text-gray-900">Rp {subtotal.toLocaleString("id-ID")}</span>
              </div>
            </div>

            {/* Baris Subtotal untuk Mobile (Layar HP) */}
            <div className="flex md:hidden justify-between items-end mt-8 border-b border-gray-100 pb-4 mb-4">
              <span className="text-lg font-normal text-gray-900">Subtotal</span>
              <span className="text-xl font-normal text-gray-900">Rp {subtotal.toLocaleString("id-ID")}</span>
            </div>
            ...
            <div className="mt-6 flex flex-col items-end text-right">
              <p className="text-[13px] text-gray-400 mb-8 font-light">
                Tax included. <span className="underline cursor-help" title="Calculated at next step">Shipping</span> calculated at checkout.
              </p>
              
              {/* . PERBAIKAN: Tombol Checkout diperkecil lebarnya (md:w-64) dan tingginya (py-3.5) */}
              <button 
