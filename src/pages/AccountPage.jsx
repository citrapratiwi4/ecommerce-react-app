              ...
              {!orders || orders.length === 0 ? (
                ...
                <div className="space-y-6">
                  {orders.map((order, index) => {
                    const itemTotal = order.product.price * (order.product.quantity || 1);
                    const shipping = order.shippingCost || 0; 
                    const discount = order.discountAmount || 0; 
                    const grandTotal = Math.max(0, itemTotal + shipping - discount); 

                    return (
                      <div key={index} className="border border-gray-200 bg-white overflow-hidden transition-all duration-300">
                        
                        {/* . PERBAIKAN: items-center dihapus, diganti jadi items-start agar foto dan teks sejajar di atas rata kiri */}
                        <div className="p-4 md:p-6 flex flex-row gap-3 md:gap-6 items-stretch">
                          ...
                          {/* . PERBAIKAN: text-center dihapus menjadi text-left permanen */}
                          <div className="flex-1 text-left pt-1">
                            ...
                            <p className="text-xs text-gray-500 mb-2 font-light">
                              ...
                              Size: {order.size || "OS"} • Qty: {order.product.quantity || 1}
                            </p>
                          ...
                          {/* . PERBAIKAN: items-center dihapus menjadi items-start (rata kiri) untuk HP, dan items-end (rata kanan) untuk Desktop */}
                          <div className="shrink-0 flex flex-col items-start md:items-end justify-between self-stretch mt-2 md:mt-0">
                            ...
                            >
                              <span>{expandedOrder === index ? "Hide Details" : "View Details"}</span>
                              <svg 
                        ...
                        {expandedOrder === index && (
                          <div className="border-t border-gray-100 bg-[#fafafa] p-6 md:p-8 animate-fade-in text-left">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12 text-sm text-gray-600">
                              ...
                              <div>
                                <div className="mb-8">
                                  <h5 className="font-medium text-gray-900 mb-3 text-xs uppercase tracking-widest">Contact information</h5>
                                  <p className="font-light">{order.shippingInfo?.email || profileData.email}</p>
                                </div>
                                <div>
                                  <h5 className="font-medium text-gray-900 mb-3 text-xs uppercase tracking-widest">Shipping address</h5>
                                  <div className="font-light space-y-1">
                                    <p className="font-normal text-gray-800">{order.shippingInfo?.name || profileData.name}</p>
                                    <p>
                                      {order.shippingInfo?.address || "Jalan Binjai - Medan"}
                                      {order.shippingInfo?.apartment ? `, ${order.shippingInfo.apartment}` : ""}
                                    </p>
                                    <p>{order.shippingInfo ? `${order.shippingInfo.subDistrict}, ${order.shippingInfo.city}` : "Kabupaten Langkat"}</p>
                                    <p>{order.shippingInfo ? `${order.shippingInfo.province} ${order.shippingInfo.postalCode}` : "North Sumatra 20773"}</p>
                                    <p>Indonesia</p>
                                    <p className="pt-2">{order.shippingInfo?.phone || "081263332861"}</p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div>
                                <div className="mb-8">
                                  <h5 className="font-medium text-gray-900 mb-3 text-xs uppercase tracking-widest">Payment method</h5>
                                  ...
                                  <div className="font-light text-xs text-gray-500 space-y-1.5 bg-white p-3 border border-gray-100 rounded-md shadow-sm mt-3">
                                    ...
                                    <div className="flex justify-between w-full max-w-[220px] font-medium text-gray-900 pt-2 border-t border-gray-100 mt-1">
                                      ...
                                      <span>Rp {grandTotal.toLocaleString("id-ID")} IDR</span>
                                    </div>
                                  </div>

                                </div>
                                <div className="mb-8">
                                  <h5 className="font-medium text-gray-900 mb-3 text-xs uppercase tracking-widest">Billing address</h5>
                                  <p className="font-light italic text-gray-500">Same as shipping address</p>
                                </div>
                                <div>
                                  <h5 className="font-medium text-gray-900 mb-3 text-xs uppercase tracking-widest">Shipping method</h5>
                                  <p className="font-light">JNE Express - REG (4 to 5 business days)</p>
                                </div>
                              </div>
