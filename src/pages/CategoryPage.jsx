...
function CategoryPage({ wishlist, toggleWishlist, cart, setIsCartOpen }) {
  ...
  const bannerImage = displayProducts.length > 0 ? displayProducts[0].image : "";

  // 2. Filter stok
  if (inStockOnly) {
    displayProducts = displayProducts.filter(
      (item) => item.category.toLowerCase() !== "blazer" && !item.isComingSoon && item.stock !== 0
    );
  }
  ...
  if (sortBy === "best-selling") {
    displayProducts = displayProducts.filter((item) => {
      const isItemComingSoon = item.category.toLowerCase() === "blazer" || item.isComingSoon;
    ...
    <div className="bg-white min-h-screen text-gray-800">
      ...
      <div className="flex flex-col md:flex-row-reverse w-full h-auto md:h-[70vh] bg-white mb-8 md:mb-12 overflow-hidden">
        <div className="w-full md:w-1/2 flex flex-col justify-center items-center md:items-start px-6 py-12 md:px-24 bg-white">
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            {/* Teks Essentials diturunkan ukurannya sedikit di HP */}
            <p className="text-[10px] md:text-[12px] tracking-[0.5em] uppercase text-gray-400 mb-4 md:mb-6 font-light">
              Élanora Essentials
            </p>
            {/* . PERBAIKAN: Ukuran font dikecilkan (text-5xl) dan jarak huruf dilegarkan (tracking-widest) khusus di HP */}
            <h2 className={`font-extralight tracking-widest md:tracking-tighter uppercase text-gray-900 leading-none ml-0 md:ml-[-7px] ${categoryName.length > 6 ? "text-4xl md:text-8xl" : "text-5xl md:text-9xl"}`}>
              {categoryName}
            </h2>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 h-[60vh] md:h-full bg-white flex items-center justify-center">
          <div className="w-full h-full flex items-center justify-center overflow-hidden">
            {bannerImage ? (
              <img
                src={
                  categoryName.toLowerCase() === "dress" ? dressbiru1 : 
                  categoryName.toLowerCase() === "skirt" ? banerskirt : 
                  categoryName.toLowerCase() === "footwear" ? baner :
                  categoryName.toLowerCase() === "blazer" ? blazerhijau :
                  bannerImage
                } 
                alt={categoryName}
          ...
          <div className="flex items-center gap-8 md:gap-12">
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative flex items-center">
                ...
                />
                <div className={`block w-9 h-5 rounded-full transition-colors duration-300 ${inStockOnly ? 'bg-gray-900' : 'bg-gray-200'}`}></div>
                <div className={`absolute left-1 bg-white w-3 h-3 rounded-full transition-transform duration-300 ${inStockOnly ? 'transform translate-x-4' : ''}`}></div>
              </div>
            ...
            <div className="relative">
              ...
              >
                <span className="text-[13px] font-light tracking-wide">Sort by</span>
                <svg 
                  ...
                  {sortOptions.map((option) => (
                    <button
                      ...
                      className={`text-left px-5 py-3 text-[10px] tracking-[0.15em] uppercase transition-colors hover:bg-gray-50 ${
                        sortBy === option.value ? 'text-gray-900 font-bold bg-gray-50' : 'text-gray-500 font-medium'
                      }`}
                    >
                      {option.label}
                    </button>
      ...
      <div className="max-w-[1400px] mx-auto px-6 pb-20">
        {displayProducts.length === 0 ? (
          <div className="text-center py-20 bg-gray-50 rounded-sm">
             <p className="text-gray-500 mb-8 font-light italic">Oops, no items match your current filter.</p>
             <button 
                onClick={() => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16 animate-fadeIn">
            {displayProducts.map((item) => {
              const isComingSoon = item.category.toLowerCase() === "blazer" || item.isComingSoon;
              ...
              return (
                <div key={item.id} className="group relative">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={wishlist.find((w) => w.id === item.id) ? "#dc2626" : "none"} stroke={wishlist.find((w) => w.id === item.id) ? "#dc2626" : "black"} strokeWidth="1.5" className="w-6 h-6 md:w-7 md:h-7 transition-colors duration-300">
                  ...
                  <Link to={isUnclickable ? "#" : `/product/${item.id}`} className={`block ${isUnclickable ? "cursor-default" : ""}`}>
                    <div className="relative w-full h-[350px] md:h-[450px] overflow-hidden bg-gray-50">
                      ...
                      {isComingSoon && (
                        <div className="absolute inset-0 bg-white/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <span className="text-[10px] tracking-[0.3em] font-medium uppercase bg-white/90 px-6 py-3 shadow-lg text-gray-700">Preview Only</span>
                        </div>
                      )}
                      ...
                      {!isComingSoon && isSoldOut && (
                        <div className="absolute inset-0 bg-white/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <span className="text-[10px] tracking-[0.3em] font-medium uppercase bg-white/90 px-6 py-3 shadow-lg text-gray-700">Sold Out</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-5 space-y-2 text-center">
                      ...
                      {isComingSoon ? (
                        <p className="text-[10px] font-bold tracking-[0.2em] text-gray-400 uppercase">Coming Soon</p>
                      ) : isSoldOut ? (
                        <p className="text-[10px] font-bold tracking-[0.2em] text-red-800 uppercase">Out of Stock</p>
                      ) : (
                        <p className="text-sm font-light text-gray-500">Rp {item.price.toLocaleString("id-ID")}</p>
                      )}
                    </div>
                  </Link>
                </div>
      ...
      <section className="py-20 md:py-32 border-t border-gray-50 text-center">
        <div className="max-w-xl mx-auto px-6">
          ...
          {/* . PERBAIKAN: Di HP dikecilkan menjadi text-base (ukuran teks normal) agar lebih kalem dan elegan. Di Laptop tetap text-2xl */}
